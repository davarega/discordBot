...
require('colors');
const webhookUrl = config.webhook.skynara;
...
function logHandler(type = "undefined", msg = "0", user = "undefined", command = "undefined", query = "undefined", error = "undefined") {
	...
	let message = {
		...
		"distube": [
			...
			`user: ${user.green} add playlist: ${command.cyan} to queue (${query.blue} songs)`,
		...
		"error": [
			...
			`user: ${user.green} try command: ${command.cyan} get some error: ${error.red}`
		],
		"undefined": ["undefined"],
	};
...
async function logEmbed(logger, type, msg, user, command, query, error) {
	...
	let message = {
		...
		"distube": [
			...
			`user: ${user} add playlist: ${command} to queue (${query} songs)`,
